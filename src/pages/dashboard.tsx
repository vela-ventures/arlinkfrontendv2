...
export default function Dashboard() {
    ...
    return <Layout>
        ...
        {managerProcess && deployments.length == 0 && <div className="text-muted-foreground mx-auto text-center">No deployments yet<br /><Link href="/deploy"><Button variant="link" className="text-muted-foreground p-0">Click here to create one</Button></Link></div>}
        <HoverEffect items={[...deployments].reverse().map((dep: TDeployment) => { return { title: dep.Name, description: dep.RepoUrl, link: `/deployments/${dep.Name}` } })} />
    </Layout>
}
